export default {
  common: {
    cancel: "Отмена",
    error: "Ошибка",
    success: "Успешно",
    save: "Сохранить",
    back: "Назад",
    edit: "Изменить",
    delete: "Удалить",
    reset: "Сбросить",
    apply: "Применить",
    warning: "Внимание",
    ok: "ОК",
    loading: "Загрузка...",
  },

  auth: {
    title: "TeamUp",
    subtitle: "Найди идеальных тиммейтов для игр",
    email: "Email",
    password: "Пароль",
    confirmPassword: "Подтвердите пароль",
    signIn: "Войти",
    signUp: "Регистрация",
    createAccount: "Создать аккаунт",
    noAccount: "Нет аккаунта? ",
    hasAccount: "Уже есть аккаунт? ",
    fillAllFields: "Заполните все поля",
    passwordsMismatch: "Пароли не совпадают",
    passwordMinLength: "Пароль должен содержать минимум 6 символов",
    invalidCredentials: "Неверный email или пароль",
    connectionError: "Ошибка соединения. Попробуйте ещё раз.",
    footer: "Продолжая, вы соглашаетесь с Условиями использования и Политикой конфиденциальности",
  },

  onboarding: {
    steps: {
      profile: "Профиль",
      games: "Игры",
      schedule: "Расписание",
      finish: "Готово",
    },
    stepOf: "Шаг {{current}} из {{total}}",
    continue: "Продолжить",
    startMatching: "Начать поиск",

    createProfile: "Создайте профиль",
    createProfileSubtitle: "Расскажите о себе, чтобы тиммейты могли вас найти",
    nickname: "Никнейм *",
    nicknamePlaceholder: "Ваш игровой ник",
    region: "Регион *",
    languages: "Языки",
    microphone: "Микрофон",
    discordTag: "Discord Tag (необязательно)",
    discordPlaceholder: "username#1234",
    bioOptional: "О себе (необязательно)",
    bioPlaceholder: "Расскажите о своём стиле игры...",

    selectGames: "Выберите игры",
    selectGamesSubtitle: "Выберите игры, для которых хотите найти тиммейтов",
    playstyle: "Стиль игры",
    platform: "Платформа",

    setSchedule: "Настройте расписание",
    setScheduleSubtitle: "Когда вы обычно доступны для игры?",
    daysAvailable: "Доступные дни",
    timeSlots: "Время",

    allSet: "Всё готово!",
    allSetSubtitle: "Начните свайпать, чтобы найти идеальных тиммейтов. Удачи и приятной игры!",

    warningGames: "Профиль создан, но игры не удалось сохранить. Вы можете добавить их позже в настройках.",
    warningSchedule: "Профиль создан, но расписание не удалось сохранить. Вы можете настроить его позже.",
    errorCreateProfile: "Не удалось создать профиль: {{message}}",
  },

  discover: {
    findingTeammates: "Ищем тиммейтов...",
    noMoreProfiles: "Больше нет профилей",
    checkBackLater: "Загляните позже — появятся новые тиммейты",
    swipesLeft: "{{count}} свайпов осталось сегодня",
    dailyLimitTitle: "Дневной лимит исчерпан",
    dailyLimitMessage: "Вы израсходовали дневной лимит свайпов. Попробуйте завтра!",
    superLikesLeft: "{{count}} супер-лайков осталось",
    superLikeLimitTitle: "Лимит супер-лайков",
    superLikeLimitMessage: "Вы использовали все супер-лайки на сегодня.",
    buySuperLikes: "Купить супер-лайки",
    boost: "Буст",
    boostActive: "Активен",
    undoErrorTitle: "Нельзя отменить",
    undoErrorMessage: "Нет недавних свайпов для отмены.",
    itsAMatch: "Совпадение!",
    matchSubtitle: "Вы и {{name}} понравились друг другу!",
    sendMessage: "Написать",
    keepSwiping: "Продолжить поиск",
    playingNow: "Играю сейчас",
    playingNowActive: "Вы онлайн",
    setPlayingNow: "Статус доступности",
    playingNowSubtitle: "Дайте другим знать, что вы готовы играть прямо сейчас",
    timeLeft: "Осталось",
    turnOff: "Отключить",
    boostErrorTitle: "Ошибка буста",
    boostErrorBody: "Не удалось изменить статус буста. Попробуйте ещё раз.",
    playingNowErrorTitle: "Ошибка статуса",
    playingNowErrorBody: "Не удалось обновить статус игры. Попробуйте ещё раз.",
  },

  matches: {
    noMatchesTitle: "Пока нет совпадений",
    noMatchesSubtitle: "Продолжайте свайпать, чтобы найти тиммейтов!",
    newMatches: "Новые совпадения",
    messages: "Сообщения",
    noMessagesHint: "Пока нет сообщений — нажмите на совпадение, чтобы написать!",
  },

  profile: {
    micOn: "Микрофон вкл.",
    games: "Игры",
    gamesCount: "{{count}} игр",
    mainGame: "Основная",
    player: "Игрок",
    addGames: "Добавьте игры для поиска тиммейтов",
    details: "Подробности",
    languagesLabel: "Языки",
    discord: "Discord",
    notSet: "Не указано",
    activityStatus: "Статус активности",
    readyToPlay: "Готов играть",
    readyToPlayActive: "Другие видят, что вы ищете тиммейтов",
    readyToPlayInactive: "Покажите другим, что вы готовы играть",
    activeForHM: "Активен ещё {{hours}}ч {{minutes}}м",
    activeForM: "Активен ещё {{minutes}}м",
    settingsSection: "Настройки",
    editProfile: "Редактировать профиль",
    filters: "Фильтры",
    settings: "Настройки",
    signOut: "Выйти",
    signOutConfirm: "Вы уверены, что хотите выйти?",
    playingNowErrorTitle: "Ошибка статуса",
    playingNowErrorBody: "Не удалось обновить статус игры. Попробуйте ещё раз.",
    setDuration: "Выберите время",
    setDurationSubtitle: "Как долго вы будете доступны для игры?",
    startPlaying: "Начать игровую сессию",
  },

  chat: {
    startConversation: "Начните общение!",
    isTyping: "{{name}} печатает...",
    messagePlaceholder: "Введите сообщение...",
    online: "в сети",
    reportUser: "Пожаловаться",
    reportWhy: "Почему вы жалуетесь на этого пользователя?",
    blockUser: "Заблокировать",
    blockConfirm: "Вы уверены, что хотите заблокировать {{name}}? Вы больше не сможете обмениваться сообщениями.",
    block: "Заблокировать",
    userBlocked: "Пользователь заблокирован",
    userBlockedMessage: "{{name}} заблокирован(а).",
    chooseAction: "Выберите действие",
    reportSubmitted: "Жалоба отправлена",
    reportSubmittedMessage: "Спасибо за помощь в поддержании безопасности сообщества.",
    failedReport: "Не удалось отправить жалобу. Попробуйте ещё раз.",
    failedBlock: "Не удалось заблокировать пользователя. Попробуйте ещё раз.",
    failedSend: "Не удалось отправить сообщение. Попробуйте ещё раз.",
    loadMore: "Загрузить ранние сообщения",
    reportInappropriate: "Неприемлемый контент",
    reportHarassment: "Оскорбления",
    reportSpam: "Спам",
    reportFake: "Фейковый профиль",
  },

  editProfile: {
    tapToChangePhoto: "Нажмите, чтобы сменить фото",
    nicknameRequired: "Укажите никнейм",
    selectRegion: "Выберите регион",
    failedUpdate: "Не удалось обновить профиль. Попробуйте ещё раз.",
    failedUpload: "Не удалось загрузить аватар",
    gifHint: "💡 Можно использовать GIF для анимированного аватара",
    voiceChatOn: "Голосовой чат включён",
    voiceChatOff: "Голосовой чат выключен",
  },

  editGames: {
    selectYourGames: "Выберите игры",
    selectGamesSubtitle: "Выберите игры, для которых хотите найти тиммейтов",
    gameDetails: "Детали игр",
    gameDetailsSubtitle: "Укажите ранг и роли для каждой игры",
    rank: "Ранг / Звание",
    rolesMultiple: "Роли (можно несколько)",
    character: "Любимый персонаж / агент",
    playstyle: "Стиль игры",
    platform: "Платформа",
    selectAtLeastOne: "Выберите хотя бы одну игру",
    failedUpdate: "Не удалось обновить игры. Попробуйте ещё раз.",
    next: "Далее",
    gamesSelected: "{{count}} выбрано",
  },

  filters: {
    games: "Игры",
    region: "Регион",
    languages: "Языки",
    playstyle: "Стиль игры",
    micRequired: "Микрофон обязателен",
    availableNowOnly: "Только онлайн",
    availableNowSubtext: "Показать только игроков, готовых играть",
    applyFilters: "Применить фильтры",
  },

  settings: {
    appearance: "Внешний вид",
    theme: "Тема",
    themeAuto: "Авто",
    themeLight: "Светлая",
    themeDark: "Тёмная",
    language: "Язык",
    languageAuto: "Авто",
    languageEn: "English",
    languageRu: "Русский",
    feedback: "Отклик",
    haptic: "Тактильный отклик",
    hapticSubtitle: "Вибрация при взаимодействии",
    sound: "Звуковые эффекты",
    soundSubtitle: "Звуки при действиях",
    account: "Аккаунт",
    email: "Email",
    changePassword: "Сменить пароль",
    deleteAccount: "Удалить аккаунт",
    deleteAccountSubtitle: "Безвозвратно удалить ваш аккаунт",
    about: "О приложении",
    version: "Версия",

    currentPassword: "Текущий пароль",
    newPassword: "Новый пароль",
    confirmNewPassword: "Подтвердите новый пароль",
    enterCurrentPassword: "Введите текущий пароль",
    enterNewPassword: "Введите новый пароль",
    confirmNewPasswordPlaceholder: "Подтвердите новый пароль",
    passwordChanged: "Пароль успешно изменён",
    passwordsMismatch: "Новые пароли не совпадают",
    passwordMinLength: "Пароль должен содержать минимум 6 символов",
    failedChangePassword: "Не удалось сменить пароль",

    deleteAccountTitle: "Удалить аккаунт",
    deleteAccountWarning: "Это безвозвратно удалит ваш аккаунт, профиль, совпадения и сообщения.",
    enterPasswordToConfirm: "Введите пароль для подтверждения",
    enterPassword: "Введите пароль",
    cannotBeUndone: "Это действие нельзя отменить. Все ваши данные будут безвозвратно удалены.",
    accountDeleted: "Аккаунт удалён",
    accountDeletedMessage: "Ваш аккаунт был удалён.",
    failedDeleteAccount: "Не удалось удалить аккаунт",
  },

  navigation: {
    discover: "Поиск",
    matches: "Совпадения",
    profile: "Профиль",
    chat: "Чат",
    editProfile: "Редактировать профиль",
    editGames: "Редактировать игры",
    filters: "Фильтры",
    settings: "Настройки",
    store: "Магазин",
  },

  store: {
    heroTitle: "Усиль свою игру",
    heroSubtitle: "Выделись с помощью Супер-лайков и дай понять идеальному тиммейту, что ты серьёзно настроен.",
    superLikePacksTitle: "Наборы супер-лайков",
    superLikes: "{{count}} Супер-лайков",
    popular: "Популярно",
    best_value: "Выгодно",
    purchaseTitle: "Подтвердите покупку",
    purchaseMessage: "Купить {{count}} супер-лайков за {{price}}?",
    confirm: "Купить",
    thankYou: "Покупка успешна!",
    thankYouMessage: "{{count}} супер-лайков добавлено на ваш счёт.",
    premiumTitle: "TeamUp Premium",
    premiumSubtitle: "Безлимитные свайпы каждый день.",
    premiumBullet1: "5 супер-лайков в день",
    premiumBullet2: "Безлимитные свайпы",
    premiumBullet3: "Смотрите, кто вас лайкнул",
    perMonth: "/ месяц",
    premiumComingSoon: "Premium-подписки скоро появятся. Следите за обновлениями!",
    boostPacksTitle: "Буст профиля",
    boostTitle: "Активировать буст",
    boostConfirmMessage: "Поднять профиль за {{price}}? Вы будете первым в ленте у всех.",
    boostActiveUntil: "Буст активен до {{time}}",
    disclaimer: "Цены указаны в демонстрационных целях. Реальные платежи не обрабатываются.",
    boostErrorTitle: "Ошибка буста",
    boostErrorBody: "Не удалось активировать буст. Попробуйте ещё раз.",
  },

  swipeCard: {
    superLikedYou: "Супер-лайк от вас",
    matchPercent: "{{score}}% совпадение",
    ready: "Готов",
    online: "Онлайн",
    mic: "Микр.",
  },

  bottomSheet: {
    compatibility: "{{score}}% совместимость",
    games: "Игры",
    availability: "Расписание",
    social: "Связь",
    like: "Лайк",
  },

  a11y: {
    like: "Нравится",
    nope: "Пропустить",
    superLike: "Супер-лайк",
    undo: "Отменить свайп",
    sendMessage: "Отправить сообщение",
    moreOptions: "Ещё",
    editAvatar: "Изменить фото профиля",
    profileInfo: "Посмотреть полный профиль",
    boost: "Переключить буст",
    playingNow: "Переключить статус игры",
  },

  gameData: {
    playstyles: {
      competitive: "Соревновательный",
      flex: "Флекс",
      casual: "Казуальный",
    },
    platforms: {
      pc: "ПК",
      ps5: "PlayStation",
      xbox: "Xbox",
      mobile: "Мобильный",
    },
    regions: {
      na: "Северная Америка",
      eu: "Европа",
      asia: "Азия",
      oce: "Океания",
      latam: "Латинская Америка",
      mena: "Ближний Восток",
    },
    languages: {
      en: "Английский",
      es: "Испанский",
      pt: "Португальский",
      ru: "Русский",
      de: "Немецкий",
      fr: "Французский",
      ko: "Корейский",
      ja: "Японский",
      zh: "Китайский",
    },
    days: {
      sun: "Вс",
      mon: "Пн",
      tue: "Вт",
      wed: "Ср",
      thu: "Чт",
      fri: "Пт",
      sat: "Сб",
    },
    timeSlots: {
      morning: "Утро",
      afternoon: "День",
      evening: "Вечер",
      night: "Ночь",
    },
    quickMessages: {
      readyToPlay: "Готов играть?",
      whatRole: "Какая роль нужна?",
      gotTime: "Есть время на пару игр?",
      addDiscord: "Добавь меня в Discord!",
      teamUpLater: "Поиграем позже?",
    },
    reportReasons: {
      harassment: "Оскорбления",
      spam: "Спам",
      fake: "Фейковый профиль",
      inappropriate: "Неприемлемый контент",
      other: "Другое",
    },
  },

  reviews: {
    title: "Отзывы",
    leaveReview: "Оставить отзыв",
    yourRating: "Ваша оценка",
    tags: "Теги (до 5)",
    comment: "Комментарий (необязательно)",
    commentPlaceholder: "Расскажите о тиммейте...",
    submit: "Отправить",
    alreadyReviewed: "Вы уже оставили отзыв",
    noReviews: "Пока нет отзывов",
    averageRating: "Средняя оценка",
    reviewCount_one: "{{count}} отзыв",
    reviewCount_few: "{{count}} отзыва",
    reviewCount_many: "{{count}} отзывов",
    successMessage: "Отзыв успешно отправлен!",
    errorMessage: "Не удалось отправить отзыв",
    ratingRequired: "Поставьте оценку",
    tags_great_teamwork: "Командный игрок",
    tags_communicative: "Общительный",
    tags_skilled: "Умелый",
    tags_calm: "Спокойный",
    tags_helpful: "Полезный",
    tags_on_time: "Пунктуальный",
    tags_fun_to_play: "Весело играть",
    tags_reliable: "Надёжный",
    tags_strategic: "Стратегический",
    tags_flexible: "Гибкий",
  },

  errors: {
    authRequired: "Войдите, чтобы продолжить",
    invalidToken: "Сессия истекла. Войдите снова",
    invalidRefreshToken: "Сессия истекла. Войдите снова",
    invalidCredentials: "Неверный email или пароль",
    usernameTaken: "Этот email уже зарегистрирован",
    userNotFound: "Пользователь не найден",
    accountBanned: "Аккаунт заблокирован",
    validationFailed: "Проверьте правильность данных",
    missingField: "Заполните обязательное поле",
    invalidFormat: "Неверный формат",
    rateLimit: "Слишком много запросов. Подождите",
    swipeLimit: "Лимит свайпов на сегодня исчерпан. Возвращайтесь завтра!",
    superLikeLimit: "Лимит суперлайков на сегодня исчерпан",
    notFound: "Не найдено",
    matchNotFound: "Матч не найден",
    profileNotFound: "Профиль не найден",
    alreadyExists: "Уже существует",
    cannotSwipeSelf: "Нельзя выполнить действие с собственным профилем",
    alreadyMatched: "Вы уже в матче",
    notInMatch: "Доступ запрещён",
    blockedUser: "Пользователь заблокирован",
    contentFiltered: "Сообщение содержит недопустимые выражения",
    internalError: "Что-то пошло не так. Попробуйте ещё раз",
    dbError: "Сервер временно недоступен. Повторяем...",
    uploadFailed: "Не удалось загрузить файл. Попробуйте снова",
  },
} as const;
